<x-app-layout>
    <div class="space-y-6">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">Logistics Center Applications</h1>
                <p class="text-sm text-gray-500 mt-1">Review applications and provision center staff accounts</p>
            </div>
        </div>
        ...
        <div class="flex flex-col xl:flex-row xl:items-center gap-3">
            <form method="GET" action="{{ route('center-applications.index') }}" class="flex flex-col lg:flex-row gap-3 flex-1">
                <div class="relative flex-1 min-w-0 sm:min-w-[220px]">
                    ...
                    <input type="text" name="search" value="{{ request('search') }}" placeholder="Search by business, owner, email, or phone..."
            ...
            @if(request('search') || request('status'))
                <a href="{{ route('center-applications.index') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-white border border-gray-200 hover:border-teal hover:text-teal-dark rounded-xl text-sm font-semibold text-gray-600 transition self-start lg:self-auto">
                    Clear Filters
                </a>
            @endif
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-100">
                    <thead class="bg-gray-50/50">
                        <tr>
                            <th class="px-6 py-3.5 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Business</th>
                            ...
                            <tr class="hover:bg-teal-light/40 transition">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <span class="text-sm font-semibold text-gray-900">{{ $application->business_name }}</span>
                                    </div>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $application->owner_name }}</td>
                            ...
                            <tr>
                                <td colspan="8" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center">
                                        ...
                                        @if(request('search') || request('status'))
                                            <a href="{{ route('center-applications.index') }}" class="mt-4 inline-flex items-center px-4 py-2 bg-white border border-gray-200 hover:border-teal hover:text-teal-dark rounded-xl text-sm font-semibold text-gray-600 transition">Clear Filters</a>
                                        @endif
                                    </div>
                                </td>
                            </tr>
